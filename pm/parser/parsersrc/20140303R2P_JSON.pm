...
sub outputline
{
	my ($whichpart,$item,@content) = @_;
	my ($fileline,$data,$i,$j,$firstdata,$seconddata);
	...
	{
		#值为response
#		LogMess("第一个 $elements",1);		
#		foreach my $idata(@{$item->{$elements}->{"data"}})
#		{
#			LogMess("xxxxx\t$idata->{password}",1);
#		}
		...
		#head部分
			for ($i = 0; $i < @content; $i++) 
			{
	 			if($i<(@content-1))
	 			{
					$fileline .=$item->{$elements}->{$content[$i]}."\|";
	 			}
	 			else 
	 			{
	 				$fileline .=$item->{$elements}->{$content[$i]};
...
#		my $lineresult= outputline("head",$obj,@head);
		my $lineresult= outputline("body",$obj,@body);
