...
sub Read_Values {
	...
	if( defined( $line ) )
	{
		# 在PIF中有||出现导致LIF文件中此位置对应的字段丢失，产生BAD文件
		# Follow two line Added by zhung on 2001-11-15
#		AudMess("处理前 '${line}'");
		# 例待处理内容 |23|JNA1   JNB1||      |1234
		
		# 处理第一个无值，处理后结果为 0|23|JNA1   JNB1||      |1234
		$line=~s/^\|/0\|/;
		# 处理其他无值字段，处理后结果为 0|23|JNA1   JNB1|0|      |1234
		$line=~s/\|\|/\|0\|/g;
		# 处理一个字段的值中间有非字符（例如空格），处理后 0|23|JNA1/JNB1|0|      |1234
		$line=~s/\|([a-zA-Z0-9]+)(\s+)([a-zA-Z0-9]+)\|/\|\1\/\3\|/g;
		# 处理值为非字符的字段，处理后 0|23|JNA1/JNB1|0|0|1234
		$line=~s/\|\s+\|/\|0\|/g;
		# 重新检查处理的行的数据，保证非空
		$line=~s/\|\|/\|0\|/g;
#		AudMess("处理后 '${line}'");
		# 以上内容由 祝乃国 添加 2001-11-30 修改后三种情况
...
sub Open_PIF_Write($) {
	...
	$self->{keys_list} = undef;
	$self->{out_buf} = "## NUOEN Parser Intermediate file\n";
	$self->{block_name} = undef;
